 ...
 */
contract DotHypeResolver is Ownable, ResolverBase, IAddrResolver, IAddressResolver, ITextResolver, IContentHashResolver, IReverseResolver {
    // Custom errors
     ...
     * @return The associated address
     */
    function addr(bytes32 node, uint coinType) public view virtual override returns (bytes memory) {
        if (!isActive(node)) {
            return "";
        }
     ...
     * @param a The address to set
     */
    function setAddr(bytes32 node, uint coinType, bytes memory a) public virtual authorised(node) onlyActive(node) {
        uint256 version = _recordVersions[node];
     ...
     */
    function setText(bytes32 node, string calldata key, string calldata value) public virtual authorised(node) onlyActive(node) {
        uint256 version = _recordVersions[node];
    ...
    function supportsInterface(bytes4 interfaceID) public view virtual override returns (bool) {
        return
            ...
            interfaceID == type(IReverseResolver).interfaceId ||
