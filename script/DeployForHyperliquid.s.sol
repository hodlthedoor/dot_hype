...
contract DeployRegistry is DeployBase {
    function run() public {
        ...
        require(
            mockOracleAddress != address(0),
            "MockOracle address not set. Run DeployOracles first and export the address."
        );
...
contract ReserveTestDomains is DeployBase {
    ...
    function printSummary(address deployer, address secondAddress) internal view {
        console.log("\n====== Deployment Summary ======");
        ...
        console.log(
            "   forge script script/RegisterReservedNames.s.sol --broadcast --rpc-url https://rpc.hyperliquid-testnet.xyz/evm --chain-id 998"
        );
        console.log("2. To check registered domains:");
        console.log(
            "   forge script script/CheckDomains.s.sol --rpc-url https://rpc.hyperliquid-testnet.xyz/evm --chain-id 998"
        );
        console.log("3. To switch to the real HypeOracle:");
        console.log(
            "   forge script script/SwitchToHypeOracle.s.sol --broadcast --rpc-url https://rpc.hyperliquid-testnet.xyz/evm --chain-id 998"
        );
    }
}
