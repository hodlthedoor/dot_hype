...
contract MockRegistry is IDotHypeRegistry {
    function register(string calldata, address, uint256) external pure returns (uint256, uint256) {
        return (0, 0);
    }

    function renew(uint256, uint256) external pure returns (uint256) {
        return 0;
    }

    function expiryOf(uint256) external view returns (uint256) {
    ...
    function nameToTokenId(string calldata label) external pure returns (uint256) {
        return uint256(keccak256(abi.encodePacked(label)));
    }

    function tokenIdToName(uint256) external pure returns (string memory) {
        return "";
    }
}
